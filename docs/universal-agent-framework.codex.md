...
> in the *Runtime binding* (§3) and the path tokens.
>
> **How to use it.** Drop this file in a fresh repo and tell the agent *"Adopt
> this framework and bootstrap the project."* It runs every command and writes
...
|---|-------|------|-------|------|
| 1 | Behavioral discipline | Karpathy (pasted into `AGENTS.md`) | installed once | *How* the agent thinks & edits |
...
| Behavioral skill | **No global auto-loader** — paste the Karpathy block *into `AGENTS.md`*; optional reference copy at `.agents/skills/karpathy-guidelines/SKILL.md` |
...
{ "attachmentFolderPath": "Context/clipped", "newLinkFormat": "relative",
  ...
  "userIgnoreFilters": ["node_modules", ".venv", ".agents/worktrees", "graphify-out"] }
```
...
*cross-session AI-interaction* knowledge (decisions, gotchas) — distinct from
the *project-domain* wiki. They don't overlap.
...
log → guardrails → playbooks → commits → index, *before acting*. Two minutes
...
fresh-context sub-agent or parallel-task waves and keep only the *conclusion* in
...
adversary) and synthesize where they *disagree* before committing. (The Claude
...
stack (`pytest`, `ruff`, `flutter analyze`, an API call, a wiki lint). In Codex,
...
happen automatically is a hook *where the runtime supports one*, otherwise an
...
shoulders. Twin: `universal-agent-framework.claude.md`.*
