...
Cohesion: 0.18
Nodes (20): answerSchema, formatTweets(), validateBulkTweets(), filterImageUrls(), getProtectedUrls(), parseResult(), removeProtectedUrls(), validateImages() (+12 more)
...
Cohesion: 0.16
Nodes (7): isRedditPostUrl(), RedditClient, getRedditUserlessToken(), RedditTokenResponse, SimpleRedditPost, SimpleRedditPostWithComments, createDirIfNotExists()
...
Cohesion: 0.18
Nodes (14): FindAndGenerateImagesAnnotation, findAndGenerateImagesGraph, findAndGenerateImagesWorkflow, embedImageInTemplate(), uploadImageBufferToSupabase(), GENERATE_IMAGE_PROMPT_TEMPLATE, generateImageCandidatesForPost(), generateImageWithNanoBananaPro() (+6 more)
...
Cohesion: 0.24
Nodes (13): filterRedditPosts(), getLangChainRedditPostsFunc(), getRedditPostsFunc(), generatePostsSubgraph(), getAfterSeconds(), saveIngestedData(), sendSlackNotification(), getRedditPostIds() (+5 more)
...
Cohesion: 0.21
Nodes (13): builder, generatePostFromMessages(), graph, IngestDataAnnotation, IngestDataConfigurableAnnotation, LangChainProduct, SimpleSlackMessageWithLinks, getChannelIdFromConfig() (+5 more)

### Community 39 - "verify-general.ts"
Cohesion: 0.22
Nodes (12): getUrlContents, getUrlContentsFunc(), RELEVANCY_SCHEMA, UrlContents, verifyGeneralContent(), getUrlContents, getUrlContentsFunc(), RELEVANCY_SCHEMA (+4 more)
...
Cohesion: 0.24
Nodes (8): generatePostOrContinue(), getPrompts(), NOTE: you should likely not have this set, unless you want to use the LangChain, TWEET_EXAMPLES, LANGCHAIN_DOMAINS, shouldExcludeGeneralContent(), shouldExcludeGitHubContent(), useLangChainPrompts()

### Community 44 - "auth-socials.ts"
Cohesion: 0.29
Nodes (10): authSocialsPassthrough(), ingestTweets(), TweetResult, getArcadeLinkedInAuthOrInterrupt(), getBasicLinkedInAuthOrInterrupt(), getLinkedInAuthOrInterrupt(), getArcadeTwitterAuthOrInterrupt(), getBasicTwitterAuthOrInterrupt() (+2 more)
...
Cohesion: 0.23
Nodes (11): ensureSignature(), getMediaFromImage(), postUploadFailureToSlack(), TODO: Refactor the post/complexPost state interfaces to use a single shared inte, reshareFromMainLinkedInAccount(), retweetFromMainAccount(), uploadPost(), UploadPostAnnotation (+3 more)

### Community 49 - "reddit/types.ts"
Cohesion: 0.15
Nodes (12): Data, Gildings, Image, MediaEmbed, Preview, RedditCommentData, Resolution, Root (+4 more)
...
Cohesion: 0.40
Nodes (6): latentSpaceLoader, latentSpaceLoaderFunc(), getUniqueArrayItems(), getLatentSpaceLinks(), NAMESPACE, putLatentSpaceLinks()
...
Cohesion: 0.31
Nodes (8): constructDescription(), ConstructDescriptionArgs, getUnknownResponseDescription(), humanNode(), TODO: Update so if the mime type is blacklisted, it re-routes to human node with, routeResponse(), RouteResponseArgs, processImageInput()
...
Cohesion: 0.29
Nodes (7): cross-env, eslint-config-prettier, @types/node, devDependencies, cross-env, eslint-config-prettier, @types/node

### Community 64 - "reinterrupt.ts"
Cohesion: 0.48
Nodes (4): getInterrupts(), redoInterrupts(), updateImageUrls(), createSupabaseClient()
...
## Knowledge Gaps
- **457 isolated node(s):** `attachmentFolderPath`, `newLinkFormat`, `useMarkdownLinks`, `alwaysUpdateLinks`, `node_modules` (+452 more)
...
- **Why does `dependencies` connect `dependencies` to `HANDOVER — universal autonomous-agent framework (Codex, portable)`, `loop-de-sloppify.md`, `graph.py`, `cheerio`, `date-fns`, `date-fns-tz`, `express-session`, `file-type`, `@google/genai`, `@googleapis/youtube`, `@langchain/community`, `@langchain/core`, `@langchain/google-vertexai-web`, `@langchain/langgraph`, `@langchain/langgraph-sdk`, `@langchain/openai`, `langsmith`, `@mendable/firecrawl-js`, `moment`, `@octokit/rest`, `passport`, `passport-twitter`, `playwright`, `sharp`, `@slack/web-api`, `snoowrap`, `@supabase/supabase-js`, `@types/snoowrap`, `xml2js`, `zod`?**
